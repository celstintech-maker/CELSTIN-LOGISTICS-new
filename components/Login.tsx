...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // LIMBO RECOVERY: If Auth exists but Profile is missing, try to recreate basic profile
      if (!profile) {
        console.warn("Limbo detected: Auth exists but profile missing. Creating recovery profile...");
      ...
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') errorMsg = 'Invalid email or password.';
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. CRITICAL: Wait for Firestore to confirm the profile is written
      // This is the step that was likely failing or timing out
      await setProfileData(userCredential.user.uid, newUserProfile);
      
      if (!isSuperAdminEmail) {
        // 4. Force Sign Out to move to success screen cleanly
        await signOut(auth);
      ...
      setIsLoading(false);
      let errorMsg = 'Registration failed. Check network.';
      if (error.code === 'auth/email-already-in-use') errorMsg = 'Email is already taken.';
      if (error.code === 'permission-denied') errorMsg = 'Database rejected profile. Contact Support.';
      setMessage({ text: errorMsg, type: 'error' });
    }
  };
  ...
  if (registrationSuccess) {
    return (
      <div className="bg-white dark:bg-slate-900 p-8 md:p-10 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-md mx-auto animate-in zoom-in-95 duration-500 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white uppercase mb-4">Registration Sent</h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8 px-4 leading-relaxed">
          Your profile is now in our system. An <span className="text-indigo-600 font-bold">Admin</span> must activate your account before you can log in.
        </p>
    ...
    <div className="bg-white dark:bg-slate-900 p-8 md:p-10 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-md mx-auto relative overflow-hidden animate-in fade-in slide-in-from-bottom-4">
      <div className="text-center mb-8">
        <h2 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white uppercase tracking-tight font-outfit">
          {view === 'login' ? 'Portal Login' : view === 'register' ? 'Join Fleet' : 'Recover Access'}
        </h2>
        <div className="h-1 w-12 bg-indigo-500 mx-auto mt-2 rounded-full"></div>
      </div>

      {message.text && (
        <div className={`p-4 mb-6 rounded-xl text-[11px] font-black leading-relaxed border animate-in slide-in-from-top-2 ${message.type === 'error' ? 'bg-rose-50 border-rose-100 text-rose-600 dark:bg-rose-500/10 dark:border-rose-500/20' : 'bg-emerald-50 border-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:border-emerald-500/20'}`}>
          {message.text}
        </div>
      )}

      {view === 'forgot' ? (
        <form onSubmit={(e) => { e.preventDefault(); setIsLoading(true); sendPasswordResetEmail(auth, emailInput).then(() => {setMessage({text: 'Check email for reset link.', type: 'success'}); setView('login');}).catch(()=>setMessage({text: 'Failed to send reset email.', type: 'error'})).finally(()=>setIsLoading(false)); }} className="space-y-4">
          ...
          <button type="submit" disabled={isLoading} className="btn-primary-v3">{isLoading ? 'Sending...' : 'Send Reset Link'}</button>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <input type="email" required placeholder="Email Address" className="form-input-v3" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} autoComplete="email" />
          <input type="password" required placeholder="Password" className="form-input-v3" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" />
          ...
          <div className="flex justify-between text-[10px] font-black text-slate-500 uppercase mt-6">
            <button type="button" onClick={() => setView('register')} className="hover:text-indigo-600">Create Account</button>
            <button type="button" onClick={() => setView('forgot')} className="hover:text-indigo-600">Forgot Password?</button>
          </div>
        </form>
      ) : (
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <input type="email" required placeholder="Email Address" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="form-input-v3" />
          ...
          <input type="password" required placeholder="Password (Min 6 chars)" value={password} onChange={(e) => setPassword(e.target.value)} className="form-input-v3" minLength={6} />
      ...
      <style>{`
        ...
        .form-input-v3:focus { border-color: #6366f1; }
        ...
        .btn-primary-v3:disabled { opacity: 0.5; cursor: not-allowed; }
      `}</style>
    </div>
  );
};
